...
threadpool<pending_work_policy, new_work_policy, Tracer, D>::~threadpool() noexcept
{
    ...
    {
        if constexpr (has_tracing_v)
            Tracer::on_allow_new_work_changing(*this);

        m_allow_new_work = false;

        if constexpr (has_tracing_v)
            Tracer::on_allow_new_work_changed(*this);

        if constexpr (has_tracing_v)
            Tracer::on_wait_for_work_almost_pushed_start(*this);
        ...
        {
            if constexpr (has_tracing_v)
                Tracer::on_wait_for_work_almost_pushed_while(*this);

            std::this_thread::sleep_for(std::chrono::milliseconds(1));
        }

        if constexpr (has_tracing_v)
            Tracer::on_wait_for_work_almost_pushed_done(*this);
    }

    if constexpr (pending_work_policy == threadpool_policy_pending_work::wait_for_work_to_finish)
    {
        if constexpr (has_tracing_v)
            Tracer::on_wait_for_pending_work_to_end_start(*this);

        while (m_pending_work_to_process)
        {
            if constexpr (has_tracing_v)
                Tracer::on_wait_for_pending_work_to_end_while(*this);

            std::this_thread::sleep_for(std::chrono::milliseconds(1));
        }

        if constexpr (has_tracing_v)
            Tracer::on_wait_for_pending_work_to_end_done(*this);
    }
    else if constexpr (pending_work_policy == threadpool_policy_pending_work::leave_work_unfinished)
    {
        if constexpr (has_tracing_v)
            Tracer::on_leave_pending_work_unfinished_start(*this);
        ...
        if constexpr (has_tracing_v)
            Tracer::on_leave_pending_work_unfinished_done(*this);
    }
    else
    {
    ...
    if constexpr (has_tracing_v)
        Tracer::on_notify_workers_to_stop_start(*this);

    m_shutting_down = true;
    m_change_in_pending_work.notify_all();

    if constexpr (has_tracing_v)
        Tracer::on_notify_workers_to_stop_done(*this);

    if constexpr (has_tracing_v)
        Tracer::on_wait_for_executing_work_to_end_start(*this);

    while (m_work_executing)
    {
        if constexpr (has_tracing_v)
            Tracer::on_wait_for_executing_work_to_end_while(*this);

        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }

    if constexpr (has_tracing_v)
        Tracer::on_wait_for_executing_work_to_end_done(*this);
    ...
    for (std::size_t thread_id = 0; thread_id < m_threads.size(); ++thread_id)
    {
        ...
        if constexpr (has_tracing_v)
            Tracer::on_worker_thread_joining(*this, static_cast<unsigned int>(thread_id));

        thread.join();

        if constexpr (has_tracing_v)
            Tracer::on_worker_thread_joined(*this, static_cast<unsigned int>(thread_id));
    }
...
void threadpool<A, B, Tracer, D>::process_all_pending()
{
    ...
    if constexpr (has_tracing_v)
        Tracer::on_process_all_pending_start(*this);
    make_worker<worker_t::wait_until_shutdown_or_no_pending_work>(m_threads.size())();
    if constexpr (has_tracing_v)
        Tracer::on_process_all_pending_done(*this);
}
...
void threadpool<A, B, Tracer, D>::process_once()
{
    ...
    if constexpr (has_tracing_v)
        Tracer::on_process_once_start(*this);
    make_worker<worker_t::do_once_if_any_pending>(m_threads.size())();
    if constexpr (has_tracing_v)
        Tracer::on_process_once_done(*this);
}
...
{
    if constexpr (has_tracing_v)
        Tracer::on_wait_all_pending_start(*this);

    while (m_pending_work_to_process)
    {
        if constexpr (has_tracing_v)
            Tracer::on_wait_all_pending_while(*this);

        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }

    if constexpr (has_tracing_v)
        Tracer::on_wait_all_pending_done(*this);
}
...
void threadpool<A, B, Tracer, D>::wait_all() const
{
    ...
    {
        if constexpr (has_tracing_v)
            Tracer::on_wait_all_while(*this);

        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }

    if constexpr (has_tracing_v)
        Tracer::on_wait_all_done(*this);
}
...
{
    if constexpr (has_tracing_v)
        Tracer::on_is_allowing_new_work(*this);
...
{
    if constexpr (has_tracing_v)
        Tracer::on_is_stopping_or_stopped(*this);
...
{
    if constexpr (has_tracing_v)
        Tracer::on_work_executed(*this, thread_index);
...
{
    if constexpr (has_tracing_v)
        Tracer::on_work_executed_by_others(*this);
...
{
    if constexpr (has_tracing_v)
        Tracer::on_work_executed_total(*this);
...
auto threadpool<A, B, Tracer, D>::push_job_new_work_forbid(F&& func, Args&&... args) -> std::optional<std::future<std::invoke_result_t<F, Args...>>>
{
    ...
    if (!m_allow_new_work)
    {
        --m_work_almost_pushed;
        if constexpr (has_tracing_v)
            Tracer::on_push_job_refused(*this);
...
{
    if constexpr (has_tracing_v)
        Tracer::on_push_task_start(*this);
...
auto threadpool<A, new_work_policy, Tracer, D>::push_task_new_work_forbid(F&& func, Args&&... args) -> bool
{
    ...
    {
        if constexpr (has_tracing_v)
            Tracer::on_push_task_refused(*this);
...
auto threadpool<A, B, Tracer, D>::make_worker(unsigned int thread_id)
{
    ...
    auto worker = [this, thread_id]() -> void {
        ...
        while (true)
        {
            ...
            {
                if constexpr (has_tracing_v)
                    Tracer::on_worker_wait_change_in_pending_work_start<type>(*this, thread_id);
                ...
                m_change_in_pending_work.wait(work_lock, [this, thread_id]() {
                    if constexpr (has_tracing_v)
                        Tracer::on_worker_wait_change_in_pending_work_while<type>(*this, thread_id);
                    return m_shutting_down || !m_pending_work.empty();
                });

                if constexpr (has_tracing_v)
                    Tracer::on_worker_wait_change_in_pending_work_done<type>(*this, thread_id);
            }
            ...
            if constexpr (has_tracing_v)
                Tracer::on_worker_grab_work_start<type>(*this, thread_id);
            auto job = std::move(m_pending_work.front());
            if constexpr (has_tracing_v)
                Tracer::on_worker_grab_work_done<type>(*this, thread_id);
            m_pending_work.pop();
            ...
            ++m_work_executing;
            if constexpr (has_tracing_v)
                Tracer::on_worker_executing_start<type>(*this, thread_id);
            std::move(job)();
            if constexpr (has_tracing_v)
                Tracer::on_worker_executing_done<type>(*this, thread_id);
        ...
        if constexpr (has_tracing_v)
            Tracer::on_worker_done<type>(*this, thread_id);
    };

    if constexpr (has_tracing_v)
        Tracer::on_make_worker_done<type>(*this, thread_id);
...
{
    if constexpr (has_tracing_v)
        Tracer::on_push_work_start(*this);
...

    {
        ...
        if constexpr (has_tracing_v)
            Tracer::on_queued_new_work_start(*this);
        m_pending_work.emplace(std::move(perform_job));
        if constexpr (has_tracing_v)
            Tracer::on_queued_new_work_done(*this);

        ++m_pending_work_to_process;
    }

    if constexpr (has_tracing_v)
        Tracer::on_notify_change_in_pending_work(*this);

    m_change_in_pending_work.notify_one();

    if constexpr (has_tracing_v)
        Tracer::on_push_work_done(*this);
}
